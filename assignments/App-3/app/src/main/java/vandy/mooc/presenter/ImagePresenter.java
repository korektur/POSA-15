    ...
    public void startProcessing() {
        if (mUrlList.isEmpty())
            ...
            for (Uri uri : mUrlList) {
                ImageGrayScaleAsyncTask imageGrayScaleAsyncTask =
                        new ImageGrayScaleAsyncTask(mDirectoryPathname, getActivityContext(), this, uri);

                ImageDownloadAsyncTask imageDownloadAsyncTask = new ImageDownloadAsyncTask(
                        getActivityContext(), imageGrayScaleAsyncTask, mDirectoryPathname,
                        this.getModel());

                imageDownloadAsyncTask.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, uri);
            }
        }
    }
